...
- Compare segment comparison views.
- Compare braking, throttle, straight, and energy deployment proxy tabs.
- Strategy-modeling telemetry-derived signals and track archetype weighting.
- Analysis traffic/position context indirectly through product views.
...
- `src` contains lightweight 2026 circuit-map annotations and display metadata used by the active route components.
...
|        6 | `bahrain`    | Bahrain International Circuit | Track path exists locally        | Medium     | Good strategy-modeling degradation benchmark                |
...
- Approximate fallback rows remain compatible with current Compare segment IDs.
- Confidence is not a probability; it is an editorial/data-quality gate.

## Mapping Strategy

1. Keep current approximate segment IDs stable for Compare compatibility.
...
Create a validator and then populate Miami with a small manually reviewed pilot set. Until that pilot is verified, Compare and Analysis should keep approximate segment language.
