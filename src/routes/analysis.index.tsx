...
export const Route = createFileRoute("/analysis/")({
  ...
  errorComponent: ({ error }) => (
    <SiteShell fullWidth>
      <div className="-mx-5 -my-8 min-h-screen bg-[#eef0e8] p-5 text-[#101414]">
        <p
          role="alert"
          className="border border-[#b5252e] bg-[#fff8f1] p-4 text-sm font-bold text-[#b5252e]"
        >
...
function AnalysisIndex() {
  ...
  const activeTheme = active
    ...
    : null;
  const activeColor = activeTheme?.accent ?? "#b5252e";

  return (
    <SiteShell fullWidth>
      <div className="-mx-5 -my-8 min-h-screen bg-[#eef0e8] text-[#101414]">
        <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-8 px-5 py-8 lg:px-8">
          <section className="grid min-h-[460px] overflow-hidden border border-[#101414] bg-[#fbfaf3] lg:grid-cols-[0.95fr_1.05fr]">
            <div className="relative min-h-[320px] bg-[#101414]">
              <img
                src="/images/raceweek-pitlane-italy.png"
                alt="Formula 1 pit lane"
                className="h-full min-h-[320px] w-full object-cover"
              />
              <div className="absolute bottom-0 left-0 right-0 grid grid-cols-3 border-t border-[#101414] bg-[#fbfaf3]">
            ...
            <div className="flex flex-col justify-between gap-8 p-5 sm:p-8 lg:p-10">
              <div className="space-y-5">
                <div className="inline-flex w-fit items-center gap-2 bg-[#101414] px-3 py-2 text-[10px] font-black uppercase tracking-widest text-[#ffffff]">
                ...
                <div>
                  ...
                  <p className="mt-5 max-w-xl text-base font-semibold leading-7 text-[#42443d] sm:text-lg">
          ...
          <section className="space-y-4">
            <div className="flex flex-col gap-2 border-b border-[#101414] pb-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
              ...
              <p className="max-w-md text-sm font-semibold text-[#55584f]">
            ...
            {visibleWeekends.length === 0 ? (
              <div className="border border-[#101414] bg-[#fbfaf3] p-6 text-sm font-bold text-[#55584f]">
...
function HeroMetric({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return (
    <div className="border-r border-[#101414] p-3 last:border-r-0">
      <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-[#6b6e66]">
...
function Signal({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-[#101414] bg-[#eef0e8] p-4">
      <span className="block text-[10px] font-black uppercase tracking-widest text-[#6b6e66]">{label}</span>
      <span className="num mt-2 block text-xl font-black uppercase text-[#101414]">{value}</span>
    </div>
  );
}
...
function ControlBar({
  ...
  return (
    <div className="home-section-enter flex flex-col gap-3 border border-[#101414] bg-[#fbfaf3] p-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 border px-3 py-2 text-[10px] font-black uppercase tracking-widest transition-colors ${
                filter === option.key
                  ? "border-[#101414] bg-[#101414] text-[#ffffff]"
                  : "border-[#aeb3a8] bg-[#eef0e8] text-[#101414] hover:border-[#101414]"
              }`}
            >
      ...
      <label className="flex min-h-10 items-center gap-2 border border-[#101414] bg-[#ffffff] px-3">
        <Search className="size-4 text-[#6b6e66]" />
        <input
          value={query}
          onChange={(event) => onQuery(event.target.value)}
          placeholder="Search race, circuit, winner"
          aria-label="Search race, circuit, winner"
          className="min-w-0 bg-[#ffffff] text-sm font-bold text-[#101414] outline-none placeholder:text-[#6b6e66] sm:w-64"
        />
      </label>
    </div>
  );
...
function Timeline({
  ...
  return (
    <div className="home-section-enter overflow-x-auto border border-[#101414] bg-[#fbfaf3] p-3">
          ...
          return (
            <button
              ...
              className={`w-28 border p-3 text-left transition-transform duration-200 hover:-translate-y-0.5 ${
                selected
                  ? "border-[#101414] bg-[#101414] text-[#ffffff]"
                  : "border-[#aeb3a8] bg-[#eef0e8] text-[#101414]"
              }`}
            >
              <span className="num block text-[10px] font-black uppercase" style={{ color: selected ? country.accent : "#6b6e66" }}>
                R{weekend.round}
              </span>
              <span className="mt-2 block truncate text-xs font-black uppercase">{weekend.circuit.split(" ")[0]}</span>
              <span className="mt-3 block h-1.5" style={{ backgroundColor: weekend.hasRace ? t.color : "#aeb3a8" }} />
            </button>
          );
        })}
      </div>
...
function ReportBoard({ weekend, accent }: { weekend: Weekend | undefined; accent: string }) {
  ...
  return (
    <section className="home-section-enter grid overflow-hidden border border-[#101414] bg-[#fbfaf3] lg:grid-cols-[1fr_0.82fr]">
      <div className="p-5 sm:p-6">
        <div className="flex flex-wrap items-center gap-2">
          <span className="num bg-[#101414] px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#ffffff]">
            Round {weekend.round}
          </span>
          <span className="num border border-[#101414] px-2 py-1 text-[10px] font-black uppercase tracking-widest">
        ...
        <p className="num mt-2 text-xs font-bold uppercase tracking-widest text-[#55584f]">{weekend.circuit}</p>
        ...
        {weekend.story ? (
          <p className="mt-5 max-w-2xl text-sm font-semibold leading-7 text-[#42443d]">{weekend.story}</p>
        ) : (
          <p className="mt-5 max-w-2xl text-sm font-semibold leading-7 text-[#42443d]">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {weekend.hasRace && weekend.slug ? (
            <Link
              ...
              className="inline-flex min-h-11 items-center gap-2 bg-[#101414] px-4 text-[11px] font-black uppercase tracking-widest text-[#ffffff] transition-transform hover:-translate-y-0.5"
            >
              Open report
          ...
          {weekend.resultsOnly ? (
            <span className="inline-flex min-h-11 items-center border border-[#101414] px-4 text-[11px] font-black uppercase tracking-widest text-[#101414]">
              Results only
            </span>
          ) : null}
        </div>
      ...
      <div className="border-t border-[#101414] bg-[#eef0e8] p-5 sm:p-6 lg:border-l lg:border-t-0">
        ...
        {weekend.hasRace ? (
          <div className="mt-4 space-y-5">
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-black uppercase tracking-widest text-[#6b6e66]">Podium</p>
              {weekend.podium.length ? (
                weekend.podium.slice(0, 3).map((driver, index) => (
                  <div key={`${driver}-${index}`} className="flex items-center gap-3">
                    <span className="num grid size-7 place-items-center bg-[#101414] text-[10px] font-black text-[#ffffff]">
                      P{index + 1}
                    </span>
                    <span className="h-2 flex-1 border border-[#aeb3a8] bg-[#fbfaf3]">
                      <span
                ...
                <p className="text-sm font-bold text-[#55584f]">Podium not stored yet.</p>
              )}
            </div>
          ...
          <div className="mt-4 grid min-h-56 place-items-center border border-[#aeb3a8] bg-[#fbfaf3] p-6 text-center">
            <div>
              <CalendarClock className="mx-auto size-8 text-[#b5252e]" />
              <p className="mt-3 text-sm font-black uppercase">Awaiting report</p>
              <p className="mt-2 text-sm font-semibold text-[#55584f]">No stored race analysis for this round yet.</p>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <span
      className={`inline-flex min-h-10 items-center justify-center border px-3 text-[10px] font-black uppercase tracking-widest ${
        active
          ? "border-[#101414] bg-[#101414] text-[#ffffff]"
          : "border-[#aeb3a8] bg-[#eef0e8] text-[#6b6e66]"
      }`}
    >
      {label}
    </span>
...
function MiniFact({ label, value, icon: Icon }: { label: string; value: string; icon: LucideIcon }) {
  return (
    <div className="border border-[#aeb3a8] bg-[#fbfaf3] p-3">
      <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-[#6b6e66]">
        <Icon className="size-3.5 text-[#b5252e]" />
        {label}
      </span>
...
function CoverageMatrix({
  ...
  return (
    <section className="home-section-enter border border-[#101414] bg-[#fbfaf3] p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="flex items-center gap-2 text-xl font-black uppercase italic">
          <Eye className="size-5 text-[#b5252e]" />
          Coverage
        </h2>
        <span className="num text-[10px] font-black uppercase tracking-widest text-[#6b6e66]">
          Q / S / R
        </span>
      </div>
            ...
            <span className="mt-1 flex gap-1">
              {[weekend.hasQuali, weekend.hasSprint, weekend.hasRace].map((isStored, index) => (
                <span
                  ...
                  style={{ backgroundColor: isStored ? team(weekend.winnerTeam).color : "#aeb3a8" }}
                />
              ))}
            </span>
          </button>
...
function TeamWins({ rows }: { rows: Array<{ key: string; name: string; color: string; wins: number }> }) {
  ...
  return (
    <section className="home-section-enter border border-[#101414] bg-[#fbfaf3] p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="flex items-center gap-2 text-xl font-black uppercase italic">
          <Trophy className="size-5 text-[#b5252e]" />
          Wins By Team
        </h2>
        <span className="num text-[10px] font-black uppercase tracking-widest text-[#6b6e66]">
          Race wins
        ...
        {rows.map((row, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="h-3 border border-[#aeb3a8] bg-[#eef0e8]">
  ...
  return (
    <div
      className={`home-section-enter group min-h-44 border p-4 text-left transition-transform duration-200 hover:-translate-y-1 ${
        active ? "border-[#101414] bg-[#101414] text-[#ffffff]" : "border-[#101414] bg-[#fbfaf3] text-[#101414]"
      }`}
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <span
              ...
              style={{ color: active ? t.color : "#6b6e66" }}
            >
              Round {weekend.round}
            </span>
          ...
          <Flag className="size-5 shrink-0" style={{ color: weekend.hasRace ? t.color : "#aeb3a8" }} />
        </div>
        <p
          className={`num mt-3 truncate text-[11px] font-bold uppercase ${
            active ? "text-[#dfe3d7]" : "text-[#55584f]"
          }`}
        >
          {weekend.circuit}
        </p>
        ...
        {weekend.hasRace && weekend.slug ? (
          <Link
            ...
            className={`inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest ${
              active ? "text-[#ffffff]" : "text-[#101414]"
            }`}
          >
            Report
