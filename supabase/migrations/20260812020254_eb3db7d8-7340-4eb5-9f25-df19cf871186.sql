drop view if exists public.v_driver_season_telemetry;
create view public.v_driver_season_telemetry as
with idx as (
...
pace as (
  select i.season, pe.driver as driver_code,
         ...
         stddev_samp(pe.normalized_pace_delta_s::numeric) filter (where abs(pe.normalized_pace_delta_s::numeric) < 2.5) as pace_consistency_s,
         count(*) filter (where abs(pe.normalized_pace_delta_s::numeric) < 2.5)::int as clean_lap_samples,
...
grant select on public.v_driver_season_telemetry to anon, authenticated, service_role;